...
case "$MARKET" in US|CN) ;; *) echo "usage: $0 US|CN 1d|1h|5m DAYS adjusted|raw|both" >&2; exit 2 ;; esac
...
if [ "${QUANT_BACKFILL_ALREADY_LOCKED:-0}" != "1" ]; then
    ...
    exec /usr/bin/flock -n "$LOCK" "$0" "$MARKET" "$INTERVAL" "$DAYS" "$PRICE_MODE"
fi

stamp="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "===== Backfill [$MARKET/$INTERVAL/$PRICE_MODE] start $stamp days=$DAYS =====" >> "$LOG"
set +e
/usr/bin/timeout --kill-after=30s "${TIMEOUT_SECONDS}s" \
    "$PYTHON" -m scripts.backfill_prices \
    --market "$MARKET" --interval "$INTERVAL" --days "$DAYS" --price-mode "$PRICE_MODE" \
    >> "$LOG" 2>&1
rc=$?
set -e
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
if [ "$rc" -eq 0 ]; then
    echo "===== Backfill [$MARKET/$INTERVAL/$PRICE_MODE] OK $end =====" >> "$LOG"
    exit 0
fi

echo "===== Backfill [$MARKET/$INTERVAL/$PRICE_MODE] FAIL $end exit=$rc =====" >> "$LOG"
