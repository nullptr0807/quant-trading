...
def test_quarantine_same_day_detection_uses_shanghai_trading_date(tmp_path, monkeypatch):
    from scripts import quarantine_cn_t1_history as q
    ...
    with sqlite3.connect(db) as con:
        ...
        con.executemany(
            "INSERT INTO trades VALUES (?,?,?,?,?,?,?,?,?,?)",
            [
                # 23:30 UTC is already the next Shanghai trading date.
                (1, "CA01", "000001.SZ", "buy", 100, 10, 0, 0,
                 "2026-07-01T23:30:00+00:00", "CN"),
                (2, "CA01", "000001.SZ", "sell", 100, 9, 0, 0,
                 "2026-07-02T02:00:00+00:00", "CN"),
            ],
        )
    monkeypatch.setattr(q, "DB", db)

    result = q.run(apply=False, backup=None)
