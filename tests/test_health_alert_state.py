def test_health_alert_state_emits_new_escalation_and_recovery(tmp_path):
    ...
    issue={'severity':'warning','check':'x','market':'US'}
    ...
    escalated=dict(issue,severity='critical')
    escalation=process({'issues':[escalated]},state)
    assert escalation['notify'][0]['transition']=='escalated'
    assert escalation['notify'][0]['previous_severity']=='warning'
